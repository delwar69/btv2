...
from .models import Notice, CommitteeMember, GalleryImage, ClubMember
...
from .models import CommitteeMember, Notice
# home page view code is here
def home(request):
    recent_notices = Notice.objects.all().order_by('-date')[:6]
    gallery = GalleryImage.objects.all()[:6]  # Limit gallery to 6 images
    featured_members = CommitteeMember.objects.filter(position__in=['President', 'Secretary', 'Finance Secretary'])
...
def all_gallery_images(request):
    all_images = GalleryImage.objects.all()  # Fetch all gallery images
    return render(request, 'club/all_gallery_images.html', {'gallery': all_images})

# View for all committee members
def all_committee_members(request):
    committee_members = CommitteeMember.objects.all()
    return render(request, 'club/all_committee_members.html', {'committee_members': committee_members})

def archived_notices(request):
    archived_notices = Notice.objects.all().order_by('-date')[6:]
    return render(request, 'club/archived_notices.html', {'archived_notices': archived_notices})
